...
const TrailerTitle = ({ title, overview }) => {
  ...
  return (
<div className='w-full h-[120%] pt-[20%] px-24 flex justify-between absolute text-white  z-40'>      <div className='h-full'>
        <h1 className='text-4xl font-bold'>{title}</h1> {/* Use displayedTitle instead of title */}
        ...
        {
          showModal && <ClipsModal clips={movieClips} onClose={handleCloseModal} />
        }
      </div>
    </div>
  );
